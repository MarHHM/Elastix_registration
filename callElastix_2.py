...
def callElastix_2(self, dataset_path, I_f_name, I_m_name, I_f_mask_name, I_m_rigidityCoeffIm_name, reg_type, n_itr_rigid, n_itr_nonRigid, n_res,
                use_rigidityPenalty, use_landmarks, I_f_landmarks, I_m_landmarks, I_deformed_name):

    os.chdir(dataset_path)
    # sitk.ProcessObject_GlobalDefaultDebugOn()       # to see detailed debug info (e.g. image readers... etc.)
    Elastix = sitk.ElastixImageFilter()
    Elastix.SetOutputDirectory(I_deformed_name.split('.')[0])               # write all registration info there
    os.makedirs(Elastix.GetOutputDirectory(), exist_ok=True)
    Elastix.SetFixedImage( sitk.ReadImage(I_f_name) )           # '72_Echo1.nii' 'DataMergeR4_t1_minus_t2.nii' 'DataMergeR4_echo1.nii'
    I_m = sitk.ReadImage(I_m_name)
    Elastix.SetMovingImage(I_m)                               # 'DataMergeR3_t1_minus_t2.nii' 'DataMergeR3_echo1.nii'
    Elastix.SetFixedMask( sitk.ReadImage(I_f_mask_name) )       # MUST be specified to avoid taking into account bgd & out-of-view noise during registration -  in the documentation, they say i usually only need to provide fixed image mask (moving image mask needed only when noisy features are close to the edge of the ROI (see 5.5 in manual))
    # Elastix.SetMovingMask( sitk.ReadImage('wholeLeg_segmented_70.labels.nii') )
    ...
    if reg_type == 'NON_RIGID' or reg_type == 'PRE_ALLIGNED_NON_RIGID':
        ...
        nonRigid_pMap['Metric'] = ['AdvancedMattesMutualInformation', 'TransformBendingEnergyPenalty']
        nonRigid_pMap['Metric0Weight'] = ['1']
        nonRigid_pMap['Metric1Weight'] = ['1']
        if eval(use_rigidityPenalty):
            nonRigid_pMap['Metric'] = nonRigid_pMap['Metric'] + ('TransformRigidityPenalty',)
            nonRigid_pMap['MovingRigidityImageName'] = [I_m_rigidityCoeffIm_name]  # 'rigidity_coeff_im_70 - rigidly aligned.nii'
            nonRigid_pMap['Metric2Weight'] = ['0.1', '0.1', '0.1','4']  # ['0.1', '0.1', '0.1', '4']   ['0.5', '0.3', '1000', '1000']
            nonRigid_pMap['DilateRigidityImages'] = ['false', 'false', 'true', 'true']
            nonRigid_pMap['DilationRadiusMultiplier'] = ['2.0']  # originaly: 2.0
            nonRigid_pMap['OrthonormalityConditionWeight'] = ['1.0']  # originaly: 1.0
            nonRigid_pMap['PropernessConditionWeight'] = ['2.0']  # originaly: 2.0
        ...
        nonRigid_pMap['FixedImageBSplineInterpolationOrder'] = ['1', '1', '1','1']  # When using a RandomCoordinate sampler, the fixed image needs to be interpolated @ each iterationnonRigid_pMap['Interpolator'] = ['BSplineInterpolator']  # use 'LinearInterpolator' for faster performance
        ...
        nonRigid_pMap['WriteTransformParametersEachResolution'] = ['false']

    if eval(use_landmarks):  # must add it as the last metric
        Elastix.SetFixedPointSetFileName(I_f_landmarks)
        Elastix.SetMovingPointSetFileName(I_m_landmarks)
    ...
    I_f_read_sitk_write_nib = nib.Nifti1Image(resultImage_arr.swapaxes(0, 2),               # swapping due to diff bet nibabel & itk image axes
                                              nib.load(I_f_name).affine)                                    # use the image resulting from Elastix registration with the affine transfrom coming from the original data (e.g. fixed im)
    I_f_read_sitk_write_nib.to_filename(f'{Elastix.GetOutputDirectory()}/{I_deformed_name}')
    # sitk.WriteImage(resultImage, f'{Elastix.GetOutputDirectory()}/{I_deformed_filename}')

#################################################################
if __name__ == '__main__':
    callElastix_2(*sys.argv)              # unpack sys.argv